          ...
          let token = jwt.sign(
            {
              ...
              userdata: resolve({
                code: code.OK,
                message: "",
                data: { token: token, profile: result },
              }),
            },
            key.JWT_SECRET_KEY,
            {
  ...
  return new Promise((resolve, reject) => {
    authModel
      ...
      .then((data) => {
        if (data) {
          let token = jwt.sign(
            { uid: result.userID, userdata: data },
            key.JWT_SECRET_KEY,
            {
              expiresIn: timer.TOKEN_EXPIRATION,
            }
      ...
      .then((data) => {
        if (data) {
          ...
          sendSMS(to, body)
            .then((response) => {
              authModel
                .saveSMS(data.email, sms_token)
                .then((result) => {
                  resolve({
                    code: code.OK,
                    message: message.SMS_CODE_SENT_SUCCESSFULLY,
                    data: {},
                  });
                })
                .catch((err) => {
...
function forgotPassword(email) {
  return new Promise((resolve, reject) => {
    authModel
      ...
      .then((data) => {
        authModel
          .saveRandomPassword(email)
          .then((randomPassword) => {
            sendMail(
              mail.TITLE_FORGOT_PASSWORD,
              email,
              mail.TYPE_FORGOT_PASSWORD,
              randomPassword
            )
              .then((response) => {
                resolve({
                  code: code.OK,
                  message: message.EMAIL_RESET_LINK_SENT_SUCCESSFULLY,
                  data: {},
                });
              })
              .catch((err) => {
                if (err.message.statusCode == code.BAD_REQUEST) {
                  reject({
                    code: code.INTERNAL_SERVER_ERROR,
                    message: message.EMIL_IS_NOT_EXIST,
                    data: {},
                  });
                } else {
                  reject({
  ...
  return new Promise((resolve, reject) => {
    authModel
      ...
      .then((data) => {
        ...
        let refresh_reset_token = jwt.sign(
          { tmpToken: tmpToken },
          key.JWT_SECRET_KEY,
          {
            expiresIn: timer.SMS_TOKEN_EXPIRATION,
        ...
        authModel
          .saveRefreshToken(token, refresh_reset_token)
          .then((resp) => {
            resolve({
              code: code.OK,
              message: message.VERIFIED_TOKEN_SUCCESSFULLY,
              data: { tmpToken: refresh_reset_token },
            });
          })
          .catch((err) => {
      ...
      .then((data) => {
        ...
        authModel
          ...
          .then((resp) => {
            resolve({
              code: code.OK,
              message: message.RESET_PASSWORD_SUCCESSFULLY,
              data: {},
            });
          })
          .catch((err) => {
...
function verifySMS(email, sms_code) {
  return new Promise((resolve, reject) => {
    authModel
      .verifySMS(email, sms_code)
      .then((data) => {
        if (data) {
          var sms_code = Math.floor(100000 + Math.random() * 900000);
          let refresh_sms_token = jwt.sign(
            { smsCode: sms_code },
            key.JWT_SECRET_KEY,
            {
              expiresIn: timer.SMS_TOKEN_EXPIRATION,
          ...
          authModel
            .saveSMS(email, refresh_sms_token)
            .then((resp) => {
              let userdata = data;
              let token = jwt.sign(
                { uid: userdata.userID, userdata: userdata },
                key.JWT_SECRET_KEY,
                {
                  expiresIn: timer.TOKEN_EXPIRATION,
                }

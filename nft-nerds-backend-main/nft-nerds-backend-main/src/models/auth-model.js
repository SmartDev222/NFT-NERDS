...
function login(authData) {
  return new Promise((resolve, reject) => {
    let query =
      "SELECT * FROM " +
      table.USERS +
      ' WHERE email = ? and permission = "active" and status = "active"';
          ...
          bcrypt.compare(
            ...
            function (error, result) {
              if (error) {
                ...
                if (result) {
                  let query =
                    "Insert into logs (userID, login_time, logout_time) values (?,?,?)";
                  db.query(
                    query,
                    [
                      rows[0].userID,
                    ...
                    (error, rows1, fields) => {
                      if (error) {
                        ...
                        let update_query =
                          "UPDATE " +
                          table.USERS +
                          " SET invitation_status = ? WHERE email = ?";
                        db.query(
                          update_query,
                          ["accepted", authData.email],
                          (error, result, fields) => {
                            if (error) {
                              reject({
...
function login_as(data) {
  return new Promise((resolve, reject) => {
    let query =
      "SELECT * FROM " +
      table.USERS +
      ' WHERE userID = ? and permission = "active" and status = "active"';
  ...
  return new Promise((resolve, reject) => {
    ...
    db.query(query, [token], (error, rows, fields) => {
      if (error) {
        ...
        if (rows.length > 0) {
          let update_query =
            "UPDATE " + table.USERS + " SET token = ? WHERE userID = ?";
          db.query(
            update_query,
            [refresh_reset_token, rows[0].userID],
            (error, rows, fields) => {
              if (error) {
                reject({ message: message.INTERNAL_SERVER_ERROR });
              } else {
